...
def generate_eventids1(ids)
  ...
  ids.each do |id, arity|
    buf << %Q[static ID pscript_id_#{id};\n]
  end
  buf << %Q[\n]
  buf << %Q[static void\n]
  buf << %Q[pscript_init_eventids1(VALUE self)\n]
  buf << %Q[{\n]
  buf << %Q[    VALUE h;\n]
  buf << %Q[    ID id;\n]
  ids.each do |id, arity|
    buf << %Q[    pscript_id_#{id} = rb_intern_const("on_#{id}");\n]
  end
  buf << %Q[\n]
  buf << %Q[    h = rb_hash_new();\n]
...
def generate_eventids2_table(ids)
  buf = ""
  buf << %Q[static void\n]
  buf << %Q[pscript_init_eventids2_table(VALUE self)\n]
  buf << %Q[{\n]
...
def read_ids2(path)
  File.open(path) {|f|
    return f.read.scan(/pscript_id_(\w+)/).flatten.uniq.sort
  }
end
